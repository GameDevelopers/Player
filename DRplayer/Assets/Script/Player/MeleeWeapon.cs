...
public class MeleeWeapon : MonoBehaviour
{
    ...
    private void HandleCollision(EnemyHealth objHealth)
    {
        ...
        if (objHealth.giveUpwardForce && Input.GetAxis("Vertical") < 0 && !playerController.isGround)
        {
        ...
        if (Input.GetAxis("Vertical") > 0 && !playerController.isGround)
        {
            //Sets the direction variable to up
    ...
    private void HandleMovement()
    {
        ...
        if (weaponCollide)
        {
            ...
            if (downwardStrike)
            {
                ...
                playerRigidbody.AddForce(direction * meleeAttackManager.upwardsForce);
            }
            else
            {
                //Propels the player backwards by the amount of horizontalForce in the meleeAttackManager script
                playerRigidbody.AddForce(direction * meleeAttackManager.defaultForce);
            }
        }
    }
